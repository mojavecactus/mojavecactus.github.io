...
export async function buildCaseReport(input,{date=new Date(),viewerImage,viewerNotes=[]}={}){
 ...
 ['Reamed depth / diameter',...['femur','tibia'].map(side=>{const d=draft.values[side],derived=(side==='tibia'&&['straight','transtibial'].includes(d.technique))||(side==='femur'&&d.technique==='outside_in')||(d.technique==='retrograde'&&d.blownCortex),depth=derived?d.ttl:d.socket;return `${n(depth)} / ${n(reamedDiameter(draft,side))} mm`;})],
 ...
 if(linkedCase(draft))para('Trans-tibial preparation: the tibial guide pin is drilled up from the anteromedial tibia to the footprint and the tibial tunnel reamed first; the femoral pin is then drilled through that tunnel with an offset aimer and the femoral socket reamed through it with the same reamer'+(finite(draft.values.tibia.diameter)?` (${n(draft.values.tibia.diameter)} mm)`:'')+'.',{size:9});
