...
(async () => {
  ...
  { const sheet = FakeSheet(); const { w, dev, errs } = await boot(sheet, creds); const F = dev.fops;
    ...
    { const X = F.xlsx('cc'); const Z = w.TBX_FFLATE.unzipSync(X.bytes); const dec = new w.TextDecoder(); const sheet = dec.decode(Z['xl/worksheets/sheet1.xml']);
      ...
      check('xlsx: a zip with the workbook parts and a Field Ops sheet, named after the title', Z['[Content_Types].xml'] && Z['xl/workbook.xml'] && Z['xl/styles.xml'] && /<sheet name="Field Ops"/.test(dec.decode(Z['xl/workbook.xml'])) && X.name === 'IT9999 SM SANDBOX SPLIT - Completed Cycle Count ' + new Date().toISOString().slice(0, 10) + '.xlsx', X.name);
      ...
      const colNum = (ref) => { let n = 0; for (const ch of ref.replace(/[0-9]+$/, '')) n = n * 26 + (ch.charCodeAt(0) - 64); return n; };
      const badRows = [...sheet.matchAll(/<row r="(\d+)">(.*?)<\/row>/g)].filter(m => { const cols = [...m[2].matchAll(/<c r="([A-Z]+)\d+"/g)].map(x => colNum(x[1])); return cols.some((c, i) => i > 0 && c <= cols[i - 1]); }).map(m => m[1]);
    ...
    { w.location.hash = '#/cc/fops'; w.dispatchEvent(new w.Event('hashchange')); await sleep(300);
      ...
      check('download: the share sheet gets one .xlsx File and nothing else (no title/text, which iOS turns into a text.txt)', shared && shared.files && shared.files.length === 1 && /Completed Cycle Count .*\.xlsx$/.test(shared.files[0].name) && shared.files[0].type.indexOf('spreadsheetml') > -1 && !('title' in shared) && !('text' in shared) && !('url' in shared), shared && JSON.stringify(Object.keys(shared)));
      ...
      check('download: without the share sheet an anchor download is used', clicked && /Completed Cycle Count .*\.xlsx$/.test(clicked.download) && clicked.href === 'blob:x', clicked && clicked.download);
      w.location.hash = '#/cc'; w.dispatchEvent(new w.Event('hashchange')); await sleep(300); }
